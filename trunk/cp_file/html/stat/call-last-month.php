...
$FG_TABLE_COL[]=array ("Статус", "disposition", "13%", "center", "", "30");
...
$FG_TABLE_COL[]=array ("Отдел-CallerID", "userfield", "14%", "center", "", "20");
$FG_TABLE_COL[]=array ("Биллинг Код", "accountcode", "10%", "center", "", "20");
        		...
        		<td align="left" bgcolor="#000033">					
      			...
      			<td align="left" bgcolor="#acbdee">
					...
					<b>Диапазон для сравнения :</b> 
				 	<select name="months_compare" class="intxt">
					<option value="6" <?php  if ($months_compare=="6"){ echo "selected";}?>>- 6 месяцев</option>
					<option value="5" <?php  if ($months_compare=="5"){ echo "selected";}?>>- 5 месяцев</option>
					<option value="4" <?php  if ($months_compare=="4"){ echo "selected";}?>>- 4 месяцев</option>
					<option value="3" <?php  if ($months_compare=="3"){ echo "selected";}?>>- 3 месяца</option>
					<option value="2" <?php  if (($months_compare=="2")|| !isset($months_compare)){ echo "selected";}?>>- 2 месяца</option>
					<option value="1" <?php  if ($months_compare=="1"){ echo "selected";}?>>- 1 месяц</option>
					</select>
					</td></tr></table>
	  			</td>
    		</tr>	
...
<tbody><tr><td bgcolor="#000000">			
	...
	<tr bgcolor="#600101">
		...
		<td align="center"><font face="verdana" size="1" color="#ffffff"><b> <acronym title="Average Connection Time">ACD</acronym> </b></font></td>
	...
	<?php  		
		...
		if ($mmax) $widthbar= intval(($data[1]/$mmax)*200);
		else $widthbar=0;
	...
	?>
     ...
     <?php 	 }	 
	 	if ($totalcall) $total_tmc_60 = sprintf("%02d",intval(($totalminutes/$totalcall)/60)).":".sprintf("%02d",intval(($totalminutes/$totalcall)%60));
		else $total_tmc_60=0;				
...
<?php  if ($posted==1){ ?>
	<center>
	<IMG SRC="graph_pie.php?min_call=<?php  echo $min_call?>&fromstatsday_sday=<?php  echo $fromstatsday_sday?>&months_compare=<?php  echo $months_compare?>&fromstatsmonth_sday=<?php  echo $fromstatsmonth_sday?>&dsttype=<?php  echo $dsttype?>&sourcetype=<?php  echo $sourcetype?>&clidtype=<?php  echo $clidtype?>&channel=<?php  echo $channel?>&resulttype=<?php  echo $resulttype?>&dst=<?php  echo $dst?>&src=<?php  echo $src?>&clid=<?php  echo $clid?>&userfieldtype=<?php  echo $userfieldtype?>&userfield=<?php  echo $userfield?>&accountcodetype=<?php  echo $accountcodetype?>&accountcode=<?php  echo $accountcode?>" ALT="График статистики">
	</center>
<?php  } ?>
</center>
